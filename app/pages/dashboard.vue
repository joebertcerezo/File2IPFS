    ...
    <div class="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20 shadow-2xl">
      ...
      <div class="bg-white/10 backdrop-blur-lg rounded-3xl p-8 border border-white/20 shadow-2xl">
        ...
        <div ref="jsonDropZone" :class="[
          ...
          <div class="flex flex-col items-center space-y-4">
            ...
            <div :class="[
              ...
              isOverDropZone ? 'bg-purple-400/20 text-purple-400' : 'bg-slate-600/50 text-slate-400'
            ]">
            ...
            <div>
              <h3 class="text-lg font-medium text-white mb-2">
                {{ isOverDropZone ? 'Drop your JSON file here!' : 'Upload JSON File' }}
              </h3>
            ...
            <!-- File Input -->
            <Input @change="handleFileSelect" type="file" accept="application/json,.json"
              class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
          </div>
        </div>

        <!-- Selected File Display -->
        <div v-if="selectedFile" class="mt-6 p-4 bg-white/5 rounded-xl border border-white/10">
          <div class="flex items-center justify-between">
            ...
            <Button @click="clearFile" variant="ghost" size="sm" class="text-slate-400 hover:text-white">
        ...
        <div v-if="selectedFile" class="mt-6 flex justify-center">
          <Button @click="upload" :disabled="pending"
            ...
            <svg v-if="pending" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" fill="none" viewBox="0 0 24 24">
        ...
        <div v-if="data && !error" class="mt-6 p-4 bg-green-500/10 border border-green-500/20 rounded-xl">
          <div class="flex items-start space-x-3">
            ...
            <div>
              <h4 class="text-green-400 font-medium mb-2">Upload Successful!</h4>
              <div class="space-y-2 text-sm">
                <p class="text-slate-300">
                  <span class="font-medium">IPFS Hash:</span>
                  <code class="bg-black/20 px-2 py-1 rounded text-green-400">{{ data.data?.IpfsHash }}</code>
                </p>
                <div class="flex space-x-2">
                  <a :href="data.ipfsUrl" target="_blank" class="text-purple-400 hover:text-purple-300 underline">
                    View on IPFS
                  </a>
                  <span class="text-slate-500">|</span>
                  <a :href="data.gatewayUrl" target="_blank" class="text-purple-400 hover:text-purple-300 underline">
                    Gateway URL
                  </a>
                </div>
              </div>
        ...
        <div v-if="error" class="mt-6 p-4 bg-red-500/10 border border-red-500/20 rounded-xl">
          <div class="flex items-center space-x-3">
            ...
            <div>
              ...
              <p class="text-slate-300 text-sm mt-1">{{ error.message || 'An error occurred during upload' }}</p>
            </div>
          </div>
        </div>
      </div>
